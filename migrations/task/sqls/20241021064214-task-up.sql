...
SELECT 
    ...
    COUNT("COURSE_BOOKING".id) AS remaining_credit
FROM "USER"
inner JOIN "CREDIT_PURCHASE"
    ON "USER".id = "CREDIT_PURCHASE".user_id
inner JOIN "COURSE_BOOKING"
    ON "USER".id = "COURSE_BOOKING".user_id AND "COURSE_BOOKING".status != '課程已取消'
...
inner join "SKILL" on "SKILL"	.id = "COACH_LINK_SKILL".skill_id	--1
inner join "COACH" on "COACH".id = "COACH_LINK_SKILL".coach_id   --2
inner join "USER" on "USER".id = "COACH".user_id 		 --3
...
select 
	...
	count("CREDIT_PURCHASE".purchased_credits) as 銷售數量
from "CREDIT_PURCHASE"
